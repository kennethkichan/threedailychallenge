...
function LoggedInView() {
    ...
    const fetchInitialData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const [problemsData, userProgress, answeredQuestionsData] = await Promise.all([
                getProblems(),
            ...
            const uniqueAgeGroups = [...new Set(problemsData.map((p: Problem) => p.age_group).filter(Boolean))] as string[];
    ...
    useEffect(() => {
        ...
        const unsubscribes = allProblems.map(problem => {
            const problemRef = doc(db, 'problems', problem.id);
            return onSnapshot(problemRef, (doc) => {
                const data = doc.data();
                if (data && data.review_status === 'disputed') {
                    setAllProblems(prev => prev.filter(p => p.id !== doc.id));
                }
            });
        });
